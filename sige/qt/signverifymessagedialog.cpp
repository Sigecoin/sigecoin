...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CSigAddress addr(base58string(ui->addressIn_SM->text().toStdString()));
    if (!addr.IsValid())
    {
...
{
    CSigAddress addr(base58string(ui->addressIn_VM->text().toStdString()));
    if (!addr.IsValid())
    {
    ...
    if (!(CSigAddress(pubkey.GetID().GetBase58addressWithNetworkPubkeyPrefix()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
