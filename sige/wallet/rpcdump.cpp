...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            "importprivkey \"sigecoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"sigecoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CSigAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CSigAddress(CScriptID(script).GetBase58addressWithNetworkScriptPrefix()), strLabel);
    } else {
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Sigecoin address or script");
    }
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CSigAddress(pubKey.GetID().GetBase58addressWithNetworkPubkeyPrefix()), strLabel);
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The sigecoin address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Sigecoin %s\n", CLIENT_BUILD);
...
UniValue processImport(const UniValue& data) {
    try {
        ...
        if (!isScript) {
            address = CSigAddress(base58string(output));
        ...
        if (isP2SH) {
            ...
            CSigAddress redeemAddress = CSigAddress(CScriptID(redeemScript).GetBase58addressWithNetworkScriptPrefix());
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CSigAddress pubKeyAddress = CSigAddress(pubKeyAddressString);
